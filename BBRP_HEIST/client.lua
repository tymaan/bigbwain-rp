        ...
        if HasScaleformMovieLoaded(scaleform) and UsingComputer then
            ...
            if GetScaleformMovieFunctionReturnBool(ClickReturn) then
                ...
                elseif Hacking and ProgramID == 86 then
                    ...
                    TriggerServerEvent('bigbwain:setDoorFreezeStatus', hackingBank, #v.Doors, false)
                    mHacking = false
                    TriggerServerEvent('esx_phone:send', 'police', ' A databreach has occured ' .. v.BankName .. 's system!', {x = v.Doors[#v.Doors].Coords.x, y = v.Doors[#v.Doors].Coords.y, z = v.Doors[#v.Doors].Coords.z})        
                    TriggerServerEvent('gcPhone:sendMessage2', 'police', ' A databreach has occured ' .. v.BankName .. 's system!')
                    TriggerServerEvent('gcPhone:sendMessage', 'police', ' A databreach has occured ' .. v.BankName .. 's system!')
                ...
                if Hacking then
                    ...
                    if lives <= 0 then
                        ...
                        Wait(5000)
                        local v = Config.BankRobbery[hackingBank]
                        TriggerServerEvent('esx_phone:send', 'police', ' An anonymous person has tried to break into ' .. v.BankName .. 's system!', {x = v.Doors[#v.Doors].Coords.x, y = v.Doors[#v.Doors].Coords.y, z = v.Doors[#v.Doors].Coords.z})        
                        TriggerServerEvent('gcPhone:sendMessage2', 'police', ' An anonymous person has tried to break into ' .. v.BankName .. 's system!')
                        TriggerServerEvent('gcPhone:sendMessage', 'police', ' An anonymous person has tried to break into ' .. v.BankName .. 's system!')
    ...
    while true do
        for k, v in pairs(Config.BankRobbery) do
            ...
            if GetDistanceBetweenCoords(coords, v.Hacking.Coords, true) <= 2.0 and v.Doors[#v.Doors].Frozen and not mHacking then
                ...
                if allowed then
                    TriggerEvent('bigbwain:helpTimed', '~INPUT_CONTEXT~ Hack', 25)
                    if IsControlPressed(0, 38) then
                        local police = nil
                        ESX.TriggerServerCallback('bigbwain:getCops', function(cops)
                            police = cops
                        end)
                        while police == nil do
                            Wait(5)
                        ...
                        if police >= v.Cops then
                            ...
                            ESX.ShowNotification('Not enough police online')
                        end
                    end
                end
                Wait(10)
...
RegisterNetEvent('bigbwain:updateMoney')
AddEventHandler('bigbwain:updateMoney', function(bank, money)
...
Citizen.CreateThread(function() 
    ...
    while true do
        ...
        for k, v in pairs(Config.BankRobbery) do
            ...
            ESX.TriggerServerCallback('bigbwain:getBox', function(box)
                sdfwa = box
            end, k)
        ...
        for k, v in pairs(Config.BankRobbery) do
            ...
            if GetDistanceBetweenCoords(coords, v.Money.BoxPosition, true) <= 1.5 and not v.Doors[#v.Doors].Frozen then
                if v.Money.Amount > 0 and bag then
                    ...
                    AddTextComponentSubstringPlayerName('~INPUT_CONTEXT~ Grab the money')
                    ...
                    if IsControlPressed(0, 38) then
                        ...
                        if police >= v.Cops then
                            ...
                            while v.Money.Amount > 0 do
                                ...
                                TriggerEvent('bigbwain:helpTimed', 'Press ~INPUT_JUMP~ to stop grabbing money', 100)
                                Wait(350)
                                TriggerServerEvent('bigbwain:takeMoney', k)
...
RegisterNetEvent('bigbwain:getTimeCl')
AddEventHandler('bigbwain:getTimeCl', function(x)
    if x then
        state = 1
    else
        state = 0
...
RegisterNetEvent('bigbwain:setDoorFreezeStatusCl')
AddEventHandler('bigbwain:setDoorFreezeStatusCl', function(bank, door, status)
    Config.BankRobbery[bank].Doors[door].Frozen = status
	print(bank, door, status)
end)

RegisterNetEvent('bigbwain:safeLooted')
AddEventHandler('bigbwain:safeLooted', function(bank, safe, toggle)
    Config.BankRobbery[bank].Safes[safe].Looted = toggle
end)

RegisterNetEvent('bigbwain:text')
AddEventHandler('bigbwain:text', function(text, x, y, time)
    drawSub(text, 4, 1, x, y, 0.5, 255, 255, 255, 255, time)
end)

RegisterNetEvent('bigbwain:helpTimed')
AddEventHandler('bigbwain:helpTimed', function(text, time)
                ...
                if GetDistanceBetweenCoords(coords, d.Coords, true) <= 2.5 and d.Frozen then
                    if hasWeld then
                        while GetDistanceBetweenCoords(GetEntityCoords(player), d.Coords, true) <= 2.5 and d.Frozen and not d.Hacking do
                            ...
                            Wait(50)
                            if allowed then
                                TriggerEvent('bigbwain:helpTimed', '~INPUT_CONTEXT~ Unlock door', 50)
                                if IsControlPressed(0, 38) then
                                    local police = nil
                                    ESX.TriggerServerCallback('bigbwain:getCops', function(cops)
                                        police = cops
                                    ...
                                    if police >= v.Cops then
                                        ...
                                        local welded = true
                                        for i = 1, 100 do
                                            TriggerEvent('bigbwain:text', '[~b~SPACE~w~] ~r~Stop Welding', 0.955, 0.97, d.Time)
                                            drawSub('Weld ~b~' .. i .. '%', 4,1,0.5,0.96,0.6,255,255,255,255, d.Time)
                                            if IsDisabledControlPressed(0, 22) then
                                                ClearPedTasks(player)
                                                blockKeys = false
                                                welded = false
                                        ...
                                        print(welded)
                                        if welded then
                                            TriggerServerEvent('bigbwain:setDoorFreezeStatus', i, j, false)
                                        end
                                        Wait(50)
                                    else
                                        ESX.ShowNotification('Not enough police online!')
...
Citizen.CreateThread(function()
    ...
    for i = 1, #Config.BankRobbery do
        for j = 1, #Config.BankRobbery[i].Doors do
            TriggerServerEvent('bigbwain:getDoorFreezeStatus', i, j)
        end
    end
    while true do
        local player = PlayerPedId()
...
RegisterNetEvent('bigbwain:removeDrillCl')
AddEventHandler('bigbwain:removeDrillCl', function(coords)
                ...
                if not s.Looted then
                    if hasDrill then
                        if GetDistanceBetweenCoords(coords, s.Coords, true) <= 1.0 and not Config.BankRobbery[i].Doors[#Config.BankRobbery[i].Doors].Frozen then
                            TriggerEvent('bigbwain:helpTimed', '~INPUT_CONTEXT~ Drill safe', 5)
                            if IsControlPressed(0, 38) then
                                local police = nil
                                ESX.TriggerServerCallback('bigbwain:getCops', function(cops)
                                    police = cops
                                ...
                                if police >= Config.BankRobbery[i].Cops then
                                    TriggerServerEvent('bigbwain:toggleSafe', i, j, true)
                                    ...
                                    while drilling do
                                        ...
                                        if not playSound or speed <= 0.1 then
                                            ...
                                            TriggerServerEvent('bigbwain:removeDrill', coords)
                                        else
                                    ...
                                    if finishedDrilling then
                                        print('Success')
                                        TriggerServerEvent('bigbwain:lootSafe', i, j)
                                    else
                                        TriggerServerEvent('bigbwain:toggleSafe', i, j, false)
                                        print('Failure')
                                    end
                                    blockKeys = false
                                    ClearPedTasks(player)
...
Citizen.CreateThread(function()
    while true do
        while drilling do
            local text = '~INPUT_CELLPHONE_UP~ Drill \n~INPUT_CELLPHONE_DOWN~ Remove Drill\n~INPUT_JUMP~ Stop Drilling'
			AddTextEntry("bigbwain_heist_drill_info_text", text)
			DisplayHelpTextThisFrame("bigbwain_heist_drill_info_text", false)
...
RegisterNetEvent('bigbwain:particleTimer')
AddEventHandler('bigbwain:particleTimer', function(time)
    Wait(time)
                ...
                if IsDisabledControlPressed(0, 172) and not finishedDrilling then
                    if temperature < 1.0 then
                        ...
                        if temperature >= 1.0 then
                            ...
                            playSound = true
                            TriggerEvent('bigbwain:particleTimer', 750)
                        end
                    else
                        playSound = false
                        local c = GetEntityCoords(drillEntity)
				...
				if not Config.BankRobbery[i].Doors[#Config.BankRobbery[i].Doors].Frozen then
                    if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), Config.BankRobbery[i].Doors[#Config.BankRobbery[i].Doors].Coords) <= 5 then
                        while GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), Config.BankRobbery[i].Doors[#Config.BankRobbery[i].Doors].Coords) <= 5 do
                            TriggerEvent('bigbwain:helpTimed', '~INPUT_CONTEXT~ Close the door', 25)
                            if IsControlJustReleased(0, 38) then
                                TriggerServerEvent('bigbwain:setDoorFreezeStatus', i, #Config.BankRobbery[i].Doors, true)
                            end
                            Wait(5)
                        end
					end
